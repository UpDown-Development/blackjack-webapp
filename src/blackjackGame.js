    ...
    return(
        <div>
            ...
            { player.hand.map((card, index) => {
                return <Typography>Card {index+1}: {card.name} of {card.suit}</Typography>
            })}
            <Typography>Score: {player.aceScore === 0 ? player.score : player.aceScore} {player.bust ? 'bust' : ''}</Typography>
            <Typography>Dealer's hand:</Typography>
            { dealer.hand.map((card, index) => {
                return <Typography>Card {index+1}: {card.name} of {card.suit}</Typography>
            })}
            <Typography>Score: {dealer.aceScore === 0 ? dealer.score : dealer.aceScore} {dealer.bust ? 'bust' : ''}</Typography>
        </div>
    )
}
